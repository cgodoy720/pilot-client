...
### 2026-04-21 — Lane A4 shipped (PR #160) 👀 in review

- **Why.** Opportunity.Type was misdiagnosed during B2 PR #144 (2026-04-19): the "Other fee for service" value JP expected to see in Bedrock is a **RecordType.Name value**, not a Type value. `RecordType.Name` is the canonical categorization field (already wired in `OpportunityEditDialog.tsx:575-613` as "Record Type" picker, and as the "Revenue Stream" filter in `PipelineFilterBar`). `Opportunity.Type` is obsolete for Pursuit's philanthropy workflow and should never have been surfaced. Full delete (not UI-only hide) prevents future devs from seeing `Type` in API responses and re-wiring it. `RenewalRepeat__c` already covers the renewal/repeat dimension.
- **Fix (backend, 6 edits + RenewalRepeat__c SOQL fix in `main.py`).** Delete `opp_type: Optional[str] = Query(...)` param, strip `:{opp_type}` from `cache_key` f-string, remove `Type,` from `get_opportunities` SOQL SELECT, delete 2-line `if opp_type:` WHERE-clause block, remove `, Type` from `search_opportunities` SOQL fields string, delete `"Type": record.get("Type"),` from search_opportunities response dict. **Also**, a second 3-pass adversarial verification caught that `get_opportunities` SOQL was missing `RenewalRepeat__c` entirely — which would have silently broken the `isRenewal` rewrite and the OpportunityEditDialog's Renewal/Repeat picker. Added `RenewalRepeat__c` to the SELECT clause (own line) + pinned by new SOQL-content test assertion.
- **Fix (frontend, 2 full-file deletions + 13 touched files).** DELETE `hooks/useOpportunityTypePicklist.ts` (86 LOC) + `components/inline-edit/cells/TypeCell.tsx` (79 LOC) — both orphaned since the B2 PR #144 column-removal. Delete `Type: string | null;` from `SalesforceOpportunity`. In `pages/Opportunities.tsx`: remove picklist import + hook call, dead `pbcOnly` state cascade, Type filter predicate, `typeOptions` IIFE, column callbacks + dep arrays. In `pages/Opportunities/useOpportunityData.ts`: simplify `oppQueryKey` signature, remove `pbcOnly` param, delete `params.opp_type = 'PBC'` branch. In `pages/Opportunities/columns.tsx`: remove dead `typeOptions?: string[]` interface field + 3 stale BUG-UI-9 comments. In `pages/Opportunities/helpers.ts`: drop orphaned `Type?: string;` from local `Opportunity` interface. In `components/PipelineFilterBar.tsx`: remove `types: string[]` from `PipelineFilters`, `types: []` from `DEFAULT_FILTERS`, Type prop + Autocomplete block + active-filter chip. In `services/api.ts`: remove `opp_type?: string` from `getOpportunities` method signature. Stale comment cleanup in `hooks/useOpportunityRecordTypes.ts`.
- **Scope-expansion note (4 surfaced consumers beyond scope doc + 1 critical backend fix).** Verification passes surfaced 4 additional consumers not listed in `tasks/opp-type-full-delete-decision.md` (written pre-A1/A3 against HEAD `4a9407a`), plus one pre-existing backend latent bug. JP approved expanding A4 scope:
  1. **`pages/Opportunities.tsx:346-348`** — Type filter predicate. Tied to scope-doc's deletion of `types: string[]` from `PipelineFilters` interface; consumer would have failed tsc.
  2. **`pbcOnly` dead state cascade** — `Opportunities.tsx:63` `useState(false)` + `useOpportunityData.ts` param + `services/api.ts:143` param + `params.opp_type = 'PBC'` branch. `setPbcOnly` is never called anywhere (verified via full-repo grep); dead state leftover from 2026-03-03 commit `26f0238a`. Cleaned per `feedback_production_discipline` "no stubs" rule.
  3. **`pages/Progress.tsx:192 isRenewal`** — `const isRenewal = (opp) => opp.Type === 'Renewal'` used in 6 forecast callsites. Rewrite to `opp.RenewalRepeat__c === 'Renewal'` aligns with canonical `utils/priorityScoring.ts:110-112` pattern and the `OpportunityEditDialog.tsx:625-628` Renewal/Repeat picklist `{'New', 'Renewal', 'Upsell'}`. Latent-bug fix. Also required swapping the local `Opportunity` interface at `Progress.tsx:49-68` (Type → RenewalRepeat__c).
  4. **`pages/Priorities.tsx:816` + `utils/priorityScoring.ts:14`** — `Type: opp.Type || ''` mapping + `Type?: string;` on `PriorityOpp` interface. Grep confirmed no consumer reads `PriorityOpp.Type`. Dropped both.
  5. **(CRITICAL) `main.py` SOQL was missing `RenewalRepeat__c`.** Second adversarial audit caught that get_opportunities SOQL never selected `RenewalRepeat__c` — pre-existing latent bug. Without the fix, A4's `isRenewal` rewrite would always return false, silently dropping all renewals from Progress-page base-case/downside forecasts. Fix: added `RenewalRepeat__c` to SOQL SELECT; added `"RenewalRepeat__c": None` to fixture; added SOQL-content test assertion. Collateral fix: resolves pre-existing OpportunityEditDialog Renewal/Repeat picker display bug.
- **Test fixture + assertion swap.** `tests/conftest.py::make_sf_opportunity`: replaced `"Type": "Other fee for service"` default with `"RecordTypeId": "012TESTRECORDTYPE01"` + `"RecordType": {"Name": "Other fee for service"}` + `"RenewalRepeat__c": None`. `tests/test_api_endpoints.py::test_get_opportunities_returns_records`: replaced Type assertion with `assert data[0]["RecordType"]["Name"] == "Other fee for service"` + new regression-guard `assert "RenewalRepeat__c" in soql` (SOQL-content pin).
- **Verification.**
  - `pytest tests/` → **20 failed, 718 passed, 22 skipped** (baseline unchanged — net test-count delta 0; 20 pre-existing failures in `test_sf_dependencies.py`).
  - `npx tsc --noEmit` → clean.
  - `CI=true npm test -- --watchAll=false` → **25/25 suites, 367/367 tests** pass.
  - Grep proof: zero Opportunity.Type stragglers; remaining `\bType\b` in `main.py` are Account.Type, Task.Type, Content-Type header only.
- **Cross-lane (B2 #159 merged mid-work).** B2 merged at `7e955d1` during A4's verification phase. A4 rebased onto new origin/dev — **zero code conflicts** (A4 ∩ B2 code files = ∅). Two doc-file merges resolved manually: `tasks/parallel-pr-lanes.md` (B2 and A4 rows both preserved; B2 flipped ✅ merged; A4 👀 in review); `tasks/jac-running-notes.md` (A4 entry on top per newest-first convention; B2 entry preserved below).
- **Numbering drift.** Plan docs estimated A4 at #159; B2 claimed #159 first. Actual A4 = **#160**.
- **Pending for you.** Review PR #160 diff (17 tracked-file changes + 2 full-file deletions). 4 commits on branch; will squash-merge to 1 on dev. Semantic change: `isRenewal` now checks `RenewalRepeat__c` (and the field is actually populated from SF now — fixes pre-existing latent bug). No migrations, no env changes. No `OpportunityEditDialog.tsx` edits (was B2 territory).

### 2026-04-21 — Lane B2 shipped (PR #159, scope-expanded + post-smoke iteration) ✅ merged
