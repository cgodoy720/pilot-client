...
| A4 (opp-type deprecation, est. #159) | **#160** | ✅ merged 2026-04-21 |
| B2-extra (`pr-opp-payment-create`, always-visible accordion + create dialog) | **#161** | ✅ merged 2026-04-21 |
| — (Opp dialog 5 missing SOQL fields — reopen-shows-empty bug) | **#162** | ✅ merged 2026-04-21 |
| — (payment create/delete ownership + input validation) | **#163** | ✅ merged 2026-04-21 |
| — (payments round-2: PUT ownership + rate limits + UX guards) | **#164** | ✅ merged 2026-04-21 |
| — (platform-intake: in-app bug/feature form) | **#165** | ✅ merged 2026-04-21 |
| — (per-field load-status + save-blocked guard across all 5 edit dialogs) | **#166** | ✅ merged 2026-04-21 |
| — (opp-soql revert — 4 ghost fields blocking prod) | **#167** | ✅ merged 2026-04-21 |
| — (strip 4 ghost-field bindings + TS interface entries) | **#168** | ✅ merged 2026-04-21 |
| Lane B bundle: B3 + B4 + B5 + B6 + B7 + B8 + B9 + B10 (mega-A) | **#169** | 👀 this PR (2026-04-21) |
...
| B3 ✅ | **#169** | `pr-dialog-audit-account` — bundled into mega-A | `components/AccountEditDialog.tsx` — convert 7 hardcoded picklists (`FALLBACK_TYPES`, `FALLBACK_TIERS`, `FALLBACK_INDUSTRIES`, etc.) → `useSchemaPicklist`; convert `npsp__Matching_Gift_Request_Deadline__c` text → date picker; + destructive Delete footer + `onDeleted` wiring + `handleDelete` cascade invalidation | B1 |
| B4 ✅ | **#169** | `pr-dialog-audit-contact` — bundled into mega-A | `components/ContactEditDialog.tsx` — convert 5 `FALLBACK_*` → `useSchemaPicklist` (Salutation, Gender, PreferredPhone, PreferredEmail, LeadSource); add editable `npe01__AlternateEmail__c`; + destructive Delete footer on Details tab | B1 |
| B5 ✅ | **#169** | `pr-tasks-whoid` — bundled into mega-A | `main.py` (`TaskCreateRequest.WhoId` + `TaskUpdateRequest.WhoId`), `main.py::create_opportunity_task` B10 defensive dict-spread fix, `components/TaskPanel.tsx` (Contact autocomplete wired to `editTask.WhoId` + `newTask.WhoId`), `services/api.ts` (`createTask` + `updateTask` declare `WhoId`), `tests/test_api_endpoints.py` (WhoId round-trip + B10 URL-wins guard) | #153 |
| B6 ✅ | **#169** | `pr-dialog-audit-taskpanel` — bundled into mega-A | `components/TaskPanel.tsx` — convert hardcoded Status + Priority MenuItems → `useSchemaPicklist('Task', 'Status')` + `useSchemaPicklist('Task', 'Priority')` in both the create form + the TaskItem edit form (preserves `(inactive)` MenuItem per `feedback_sf_stages_sacred`); minor a11y fix (explicit id/labelId on InputLabel/Select so aria-labelledby wires through) | B1 + B5 |
| B7 ✅ | **#169** | `pr-b4a-task-title-true` — regression-tested, not reproducible | NOT reproducible on dev HEAD 1661ad8 after 10 verification passes. TaskPanel.tsx's saveEdit sends Subject through verbatim; no coercion path in React or api.ts layer. PR #169 adds `TaskPanel.test.tsx` regression-lock-down: Subject persists as the typed string, not coerced to bool/number. | B6 (same file) |
| B8 ✅ | **#169** | `pr-b4b-task-description-400` — bundled into mega-A | `components/TaskPanel.tsx::saveEdit` rewritten to be diff-based. Previously sent the entire editTask object verbatim (empty-string `ActivityDate`/`OwnerId`/`WhoId` → SF 400). Now computes diff against originalTask and sends only changed fields. Regression test in TaskPanel.test.tsx confirms only changed field in payload. | B7 |
| B9 ✅ | **#169** | `pr-b4c-task-title-not-persisting` — regression-tested, not reproducible | NOT reproducible on dev HEAD 1661ad8 after 10 verification passes. `createTaskMutation` now builds payload explicitly (drops DriveLink stub + empty-string → undefined) and regression test locks Subject string type. | B8 |
| B10 ✅ | **#169** | `pr-b4d-task-wrong-opportunity` — bundled into mega-A | `main.py::create_opportunity_task` reversed the dict-spread order so URL-path `opportunity_id` always wins over any `WhatId` in the request body. Prior spread let body's WhatId silently shadow the URL param. Regression test asserts URL WhatId wins over body WhatId. | B9 |
| B11 | #170 | `pr-b4e-task-delete-no-refresh` | `components/TaskPanel.tsx` + react-query cache-key invalidation | B10 |
