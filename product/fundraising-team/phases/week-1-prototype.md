...
**Repo:** This repo only. **App:** `financial_forecasting` — the **existing** app in this repo, which uses **React + MUI + FastAPI**. We use this stack for week 1 on purpose: it already has Salesforce integration and we ship without a new codebase. The prospect-dashboard stack (React + Vite + Tailwind + shadcn/ui) in `raw-prds/prospect-dashboard/CLAUDE.md` and the tech-stack decision applies to **net-new prospect-dashboard** work or a future standalone build; it does **not** apply to the week-1 prototype in `financial_forecasting`. **New page:** `financial_forecasting/frontend/src/pages/WeeklyPriorities.tsx`. **Route/nav:** Add `/weekly-priorities` in `App.tsx` and a nav item in `Layout.tsx`. **CSV:** Parse in browser (e.g. PapaParse); no new backend. **Grants:** Use existing `apiService.getOpportunities()`; filter client-side for close date in next 7 days (stub if API lacks close date). **State:** In-memory React state; optional IndexedDB. **Types:** Optional `types/weeklyPriorities.ts` for Prospect, Grant, WeeklyPriorityItem.
