...
import { X, AlertTriangle, MessageSquare, UserCheck, Plus, Search, Loader2, Users, BookOpen, ThumbsUp, ThumbsDown, Minus, ChevronDown, Flag } from 'lucide-react';
...
const BuilderTab = ({ builder, cohortId, onSaved, onClose }) => {
  ...
  const addBuilder = (b) => { setInvolvedBuilders(prev => [...prev, b]); setSearchQuery(''); setSearchResults([]); };
  const removeBuilder = (userId) => setInvolvedBuilders(prev => prev.filter(b => b.user_id !== userId));
    ...
    <div className="space-y-5 px-6 py-5 overflow-y-auto flex-1">
        ...
        <div>
          ...
          {primaryBuilder ? (
            ...
            <div className="relative mt-1.5">
              ...
              <input type="text" value={primarySearchQuery} onChange={(e) => setPrimarySearchQuery(e.target.value)}
                placeholder="Search for a builder..." autoFocus
              ...
              {primarySearchResults.length > 0 && (
                <div className="absolute z-10 w-full mt-1 bg-white border border-[#E3E3E3] rounded-md shadow-lg max-h-32 overflow-y-auto">
                  {primarySearchResults.map(b => (
                    <button key={b.user_id} type="button"
                      onClick={() => { setPrimaryBuilder(b); setPrimarySearchQuery(''); setPrimarySearchResults([]); }}
      ...
      <div>
        ...
        <div className="relative">
          ...
          {searchResults.length > 0 && (
            <div className="absolute z-10 w-full mt-1 bg-white border border-[#E3E3E3] rounded-md shadow-lg max-h-32 overflow-y-auto">
              {searchResults.map(b => (
                <button key={b.user_id} type="button" onClick={() => addBuilder(b)}
                  className="w-full text-left px-3 py-2 text-xs hover:bg-[#EFEFEF] transition-colors">
                  {b.first_name} {b.last_name} <span className="text-slate-400">{b.email}</span>
                </button>
              ))}
      ...
      {logType !== 'interview' && (
        <div>
          ...
          <Textarea value={bondBlocks} onChange={(e) => setBondBlocks(e.target.value)} placeholder="Optional..."
      ...
      <div>
        ...
        <Textarea value={nextSteps} onChange={(e) => setNextSteps(e.target.value)}
          placeholder="Action items, referrals..." className="mt-1.5 min-h-[60px] text-sm" />
      </div>
        ...
        {showSupport && (
          <div className="mt-3 space-y-3 p-3 bg-[#FAFAFA] rounded-lg border border-[#E3E3E3]">
            ...
            <div>
              ...
              <Textarea value={supportDisclosure} onChange={(e) => setSupportDisclosure(e.target.value)}
                placeholder="Confidential details..." className="mt-1 min-h-[60px] text-sm" />
            </div>
      ...
      <div className="flex justify-end gap-2 pt-2">
        ...
        <Button onClick={handleSave} disabled={saving || !notes.trim() || !effectiveBuilder}
...
const CURRICULUM_STATUSES = [
  ...
  },
  {
    ...
    hover: 'hover:border-red-300 hover:text-red-600',
  },
];

// Collapsible toggle section
const ToggleSection = ({ label, icon: Icon, iconColor = 'text-slate-400', enabled, onToggle, children, flagMode = false }) => (
  <div className={`rounded-lg border transition-all ${enabled ? 'border-[#4242EA]/30 bg-[#4242EA]/[0.02]' : 'border-[#E3E3E3] bg-white'}`}>
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span className={`text-sm font-medium transition-colors ${enabled ? 'text-[#1E1E1E]' : 'text-slate-500'}`}>{label}</span>
        {flagMode && enabled && (
          <span className="flex items-center gap-1 text-[10px] font-semibold text-amber-600 bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded-full">
            <Flag size={9} /> Action Required
          </span>
        )}
      </div>
      <div className={`w-9 h-5 rounded-full transition-colors relative flex-shrink-0 ${enabled ? 'bg-[#4242EA]' : 'bg-slate-200'}`}>
...
const CohortTab = ({ cohortId, cohorts, onSaved, onClose }) => {
  ...
  const [nextSteps, setNextSteps]                             = useState('');
  const [nextStepsActionRequired, setNextStepsActionRequired] = useState(false);
  const [saving, setSaving]                                   = useState(false);
    ...
    try {
      const res = await fetch(`${API_URL}/api/admin/dashboard/cohort-logs`, {
        ...
        body: JSON.stringify({
          ...
          next_steps: nextSteps.trim() || null,
          next_steps_action_required: nextStepsActionRequired,
        }),
      });
      const data = await res.json();
      if (data.success) { onSaved?.(); onClose(); }
    ...
    <div className="space-y-4 px-6 py-5 overflow-y-auto flex-1">
      ...
      <div>
        ...
        <div className="relative mt-1.5">
          ...
          />
          {selectedCohort && (
            <span className="absolute right-2.5 top-2 text-[10px] font-semibold text-[#4242EA] bg-[#4242EA]/10 px-1.5 py-0.5 rounded">
              Selected
            </span>
          )}
            ...
            <div className="absolute z-10 w-full mt-1 bg-white border border-[#E3E3E3] rounded-md shadow-lg max-h-36 overflow-y-auto">
              {filteredCohorts.map(c => (
                <button key={c.cohort_id} type="button"
                  ...
                  className={`w-full text-left px-3 py-2 text-xs hover:bg-[#EFEFEF] transition-colors ${c.cohort_id === selectedCohortId ? 'bg-[#4242EA]/5 text-[#4242EA] font-medium' : ''}`}>
                  {c.name}
                </button>
              ))}
            </div>
      ...
      <div className="rounded-lg border border-[#E3E3E3] p-4 space-y-3">
        ...
        <div className="flex gap-2">
          {CURRICULUM_STATUSES.map(({ value, label, icon: Icon, active, hover }) => (
            <button
              ...
              className={`flex-1 flex flex-col items-center gap-1.5 px-2 py-3 rounded-lg border text-xs font-medium transition-all ${
                curriculumStatus === value ? active : `bg-white border-[#E3E3E3] text-slate-400 ${hover}`
              }`}
            >
              <Icon size={18} />
        ...
        <div>
          ...
          <Textarea
            value={curriculumStatusNotes}
            onChange={(e) => setCurriculumStatusNotes(e.target.value)}
      ...
      <ToggleSection
        label="Changes/Updates to Day's Curriculum"
        icon={ChevronDown}
        iconColor="text-[#4242EA]"
        enabled={todayEnabled}
        onToggle={() => setTodayEnabled(v => !v)}
      >
        <Textarea
          value={todayNotes}
      ...
      <ToggleSection
        label="Changes/Updates for Next Day's Curriculum"
        icon={ChevronDown}
        iconColor="text-[#4242EA]"
        enabled={nextDayEnabled}
        onToggle={() => setNextDayEnabled(v => !v)}
      >
        <Textarea
          value={nextDayNotes}
      ...
      <div>
        <div className="flex items-center justify-between mb-1.5">
          ...
          <label className="flex items-center gap-1.5 cursor-pointer select-none">
            <input
              type="checkbox"
              checked={nextStepsActionRequired}
              onChange={(e) => setNextStepsActionRequired(e.target.checked)}
        ...
        <Textarea
          ...
          placeholder="Action items, follow-ups, referrals..."
...
const BuilderLogModal = ({ open, onOpenChange, builder, cohortId, cohorts, onSaved, mode = 'sheet' }) => {
  ...
  const title = activeTab === 'builder'
    ? (builder
        ? 'Log Entry — ' + (builder.name || (builder.first_name + ' ' + builder.last_name))
        : 'New Facilitator Log')
    : 'New Cohort Log';
  ...
  return (
    <Wrapper open={open} onOpenChange={onOpenChange}>
      <Content className={contentClass} {...contentProps}>
        ...
        {/* Tab bar */}
        <div className="flex gap-0 px-6 pt-4 pb-0 border-b border-[#E3E3E3]">
          {[
