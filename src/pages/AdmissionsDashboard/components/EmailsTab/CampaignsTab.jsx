...
export default function CampaignsTab({ token }) {
  ...
  const fetchCampaigns = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API}/api/admissions/campaigns`, {
  ...
  const fetchEmailLists = useCallback(async () => {
    const res = await fetch(`${API}/api/admissions/leads/email-lists`, {
      headers: { Authorization: `Bearer ${token}` }
    });
  ...
  useEffect(() => {
    fetchCampaigns();
    fetchEmailLists();
  }, [fetchCampaigns, fetchEmailLists]);
  ...
  const openAnalytics = async (campaign) => {
    ...
    try {
      const res = await fetch(`${API}/api/admissions/campaigns/${campaign.campaign_id}/analytics`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`${API}/api/admissions/campaigns/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleSend = async (id) => {
    ...
    try {
      const res = await fetch(`${API}/api/admissions/campaigns/${id}/send`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  if (analyticsFor) {
    ...
    const chartData = s ? [
      ...
      { name: 'Converted', value: parseInt(s.converted), color: '#8b5cf6' },
    ] : [];

    return (
      <div className="p-4 space-y-4">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="sm" className="font-proxima gap-1" onClick={() => { setAnalyticsFor(null); setAnalyticsData(null); }}>
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                { label: 'Conversions', value: fmtPct(s.conversion_rate), sub: `${s.converted} converted` },
              ].map(stat => (
                <Card key={stat.label}>
  ...
  return (
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-proxima font-semibold text-lg">Email Campaigns</h2>
          <p className="text-sm text-gray-500 font-proxima">Build and send targeted emails to your leads.</p>
        </div>
                    ...
                    <TableRow key={c.campaign_id}>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1">
                          {c.status === 'sent' && (
                            <Button variant="ghost" size="icon" className="h-7 w-7" title="Analytics" onClick={() => openAnalytics(c)}>
                              <BarChart2 className="h-3.5 w-3.5 text-[#4242ea]" />
                            </Button>
                          )}
                          {(c.status === 'draft' || c.status === 'scheduled') && (
                            <>
                              <Button variant="ghost" size="icon" className="h-7 w-7" title="Edit" onClick={() => { setEditingCampaign(c); setBuilderOpen(true); }}>
                                <Edit2 className="h-3.5 w-3.5 text-gray-500" />
                              </Button>
                              <Button variant="ghost" size="icon" className="h-7 w-7" title="Send Now" onClick={() => handleSend(c.campaign_id)}>
                                <Send className="h-3.5 w-3.5 text-green-600" />
                              </Button>
                              <Button variant="ghost" size="icon" className="h-7 w-7" title="Delete" onClick={() => handleDelete(c.campaign_id)}>
