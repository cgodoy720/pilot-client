...
const CalendarHeader = ({ 
  ...
  // Show enrollment dropdown for regular users with multiple enrollments
  const showEnrollmentDropdown = userRole !== 'staff' && userRole !== 'admin' && enrollments.length > 1;

  return (
    <div className="flex flex-col">
      {/* Top Bar */}
      <div className="h-[45px] bg-bg-light border-b border-divider flex items-center justify-between px-[25px]">
        {/* Left: Month/Year Title and Cohort Dropdown */}
        <div className="flex items-center gap-3">
          ...
          {/* Cohort Dropdown for regular users with multiple enrollments */}
          {showEnrollmentDropdown && (
            <Select 
              value={selectedCohort || ''} 
              onValueChange={onSelectedCohortChange}
            >
              <SelectTrigger className="w-[180px] h-[32px] bg-white rounded-[5px] px-[10px] border-0 text-[16px] leading-[18px] font-proxima font-normal text-carbon-black">
                <SelectValue placeholder="Select cohort" />
              </SelectTrigger>
              <SelectContent>
                {enrollments.map((enrollment) => (
                  <SelectItem 
                    key={enrollment.cohort_id} 
                    value={enrollment.cohort_name}
                    className="text-[16px] leading-[18px] font-proxima font-normal"
                  >
                    {enrollment.cohort_name} {enrollment.is_active && '(Active)'}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
